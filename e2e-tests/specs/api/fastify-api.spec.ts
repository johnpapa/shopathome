/**
 * Fastify API Server Tests
 * Tests all Fastify API endpoints for the Shop at Home API
 */

import { test, expect } from '@playwright/test';
import { apis } from '../../config/test-config';

const fastifyUrl = apis.fastify.baseUrl;

test.describe('Fastify API Server Tests', () => {
  
  test('should have Fastify API accessible', async ({ request }) => {
    // Test if API is running (may not be in all environments)
    try {
      const response = await request.get(`${fastifyUrl}/api/products`);
      expect([200, 404, 500]).toContain(response.status());
    } catch (error) {
      console.log('Fastify API not available:', error);
      test.skip();
    }
  });
  ...
  test('POST /api/products should create a new product', async ({ request }) => {
    try {
      const newProduct = {
        name: 'Test Fastify Product',
        price: 79.99,
        category: 'Test',
        description: 'Test product for Fastify',
      };

      const response = await request.post(`${fastifyUrl}/api/products`, {
        data: newProduct,
        headers: {
      ...
      // Accept 201 (Created) or 405 (Method not allowed)
  ...
  test('PUT /api/products/:id should update a product', async ({ request }) => {
    try {
      const updatedProduct = {
        id: 1,
        name: 'Updated Fastify Product',
        price: 89.99,
        category: 'Updated',
      };

      const response = await request.put(`${fastifyUrl}/api/products/1`, {
        data: updatedProduct,
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  test('DELETE /api/products/:id should delete a product', async ({ request }) => {
    try {
      const response = await request.delete(`${fastifyUrl}/api/products/999`, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
  ...
  test('GET /api/discounts should return discounts list', async ({ request }) => {
    try {
      const response = await request.get(`${fastifyUrl}/api/discounts`);
  ...
  test('should handle CORS headers correctly', async ({ request }) => {
    try {
      ...
      console.log('Fastify CORS header:', corsHeader);
      
      // Fastify is configured with CORS
  ...
  test('should handle invalid endpoints gracefully', async ({ request }) => {
    try {
      const response = await request.get(`${fastifyUrl}/api/invalid-endpoint`);
  ...
  test('should respond within acceptable time', async ({ request }) => {
    try {
      ...
      console.log(`Fastify API response time: ${responseTime}ms`);
  ...
  test('should have proper content-type headers', async ({ request }) => {
    try {
      ...
      console.log('Content-type test not applicable');
      test.skip();
    }
  });
  ...
  test('should handle concurrent requests', async ({ request }) => {
    try {
      ...
      const requests = [
        request.get(`${fastifyUrl}/api/products`),
        request.get(`${fastifyUrl}/api/products`),
        request.get(`${fastifyUrl}/api/products`),
      ];
      ...
      console.log('Concurrent requests test not applicable');
      test.skip();
    }
  });
});
